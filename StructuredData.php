 ...
 */
class StructuredData implements \ArrayAccess, \Countable, \Iterator
{
    public $error;
    private $datasets = 0;
    private $key_field = 0;
            ...
            foreach ($data as $item) {
                foreach (array_keys($item) as $key) {
                    if ($key !== $this->key_field
                      && array_search($key, $this->dataset_fields, true) === false) {
     ...
     *
     * @param mixed $values
     * @param mixed $force_assoc
     * @param mixed $datetime_keys
     * @param mixed $int_keys
     */
    public static function convertFrom(
        $values,
        $force_assoc,
        $datetime_keys,
        $int_keys,
    ...
    public function revalue($datasets, $callback)
    {
        ...
        foreach ($this->data as $index => $item) {
            ...
            // get updated row
            $new_row = $callback($item[$this->key_field], $row);

            for ($i = 0; $i < $datasets; ++$i) {
                $this->data[$index][$new_values[$i]] = $new_row[$i] ?? null;
            }
        }
     ...
     * @param mixed $dataset
     */
    public function transform($callback, $dataset = 0): void
    {
     ...
     * @param mixed $name
     */
    public function addField($name): void
    {
     ...
     *
     * @param null|mixed $insert
     */
    public function addRow(array $row, $insert = null): void
    {
     ...
     * @param mixed $reverse
     */
    public function sort($dataset, $reverse): void
    {
        $key = $this->dataset_fields[$dataset];
     ...
     * @param mixed $data
     */
    private function scatter2DDatasets(&$data): void
    {
        $newdata = [];
        ...
        foreach ($data as $dataset) {
            foreach ($dataset as $item) {
                if (isset($item[$key_field], $item[$value_field])) {
