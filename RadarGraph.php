...
class RadarGraph extends LineGraph
{
     ...
     * @param mixed $w
     * @param mixed $h
     * @param mixed $reverse_length
     */
    public function guidelinePathAbove(
        $axis,
        $value,
        &$x,
        &$y,
     ...
     * @param mixed $reverse_length
     */
    public function guidelinePathBelow(
        $axis,
        $value,
        &$x,
        &$y,
        ...
        if ($axis === 'x') {
            ...
            if ($reverse_length) {
                [$p1x, $p1y] = $this->transformCoords($x, $this->radius - $h);
                $h = $this->radius;
            } else {
                $p1x = $this->xc;
                $p1y = $this->yc;
            ...
            [$p2x, $p2y] = $this->transformCoords($x, $h);
    ...
    protected function grid()
    {
        ...
        $back = $subpath = '';
        $back_colour = new Colour(
            $this,
            $this->getOption('grid_back_colour'),
            true,
            true,
            true,
        );
