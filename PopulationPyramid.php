...
class PopulationPyramid extends HorizontalStackedBarGraph
{
                ...
                if ($this->getOption('show_bar_totals')) {
                    if ($xpos) {
                        ...
                        if (\is_callable($this->getOption('bar_total_callback'))) {
                            $bar_total = ($this->getOption('bar_total_callback'))(
                                $item->key,
                                $xpos
                            );
                        } else {
    ...
    protected function getAxes($ends, &$x_len, &$y_len)
    {
        ...
        $y_units_before = '';
        $x_decimal_digits = $this->getOption(
            ['decimal_digits_y', 0],
            'decimal_digits',
        );
        $y_decimal_digits = $this->getOption(
            ['decimal_digits_x', 0],
            'decimal_digits',
        );
        $x_text_callback = $this->getOption(
            ['axis_text_callback_x', 0],
            'axis_text_callback',
        );
        $y_text_callback = $this->getOption(
            ['axis_text_callback_y', 0],
            'axis_text_callback',
        );
        ...
        if (!is_numeric($grid_division_h)) {
            $x_min_space = $this->getOption(
                ['minimum_grid_spacing_h', 0],
        ...
        $y_axis_factory = new AxisFactory(
