...
class LineGraph extends PointGraph
{
    ...
    public function drawLine($dataset, $points, $y_bottom)
    {
        ...
        if (\count($points) > 1) {
            ...
            if ($curve) {
                [$path, $fillpath] = $this->getCurvedLinePath($line_points, $y_bottom, $last_x, $curve);
            } else {
                [$path, $fillpath] = $this->getLinePath($line_points, $y_bottom, $last_x);
            }
    ...
    {
        $style_info['line_style'] = $this->line_styles[$dataset] ?? null;
        $style_info['fill_style'] = $this->fill_styles[$dataset] ?? null;
