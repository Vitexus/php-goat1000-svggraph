...
class Guidelines
{
    ...
    public function __construct(&$graph, $flip_axes, $assoc, $datetime)
    {
        ...
        $this->text_colour = new Colour(
            $graph,
            $graph->getOption('guideline_text_colour', 'guideline_colour'),
        );
        $this->text_opacity = $graph->getOption(
            'guideline_text_opacity',
            'guideline_opacity',
        );
     ...
     * @param mixed $g
     */
    protected function calculate($g): void
    {
        if (!\is_array($g)) {
            $g = [$g];
        }
        ...
        if ($axis === 'x') {
            ...
            } elseif ($this->assoc_keys) {
                // $value is a key - must be converted later when the axis
                // has been created
            }
        }

        $above = $g['above'] ?? $this->above;
        $position = $above ? self::ABOVE : self::BELOW;
        $guideline = [
            'value' => $value,
            'depth' => $position,
            'title' => $g[1] ?? '',
     ...
     * @param mixed $d
     */
    protected function buildGuideline(&$line, &$lines, &$text, &$path, &$d): void
    {
        $length = $this->length;
        ...
        if (!empty($line['title'])) {
            ...
            [$text_w, $text_h] = $svg_text->measure(
                $line['title'],
                $font_size,
                $text_angle,
                $line_spacing,
            ...
            [$x, $y, $text_pos_align] = Graph::relativePosition(
                $text_pos,
                $y,
                $x,
                $y + $h,
     ...
     */
    protected function guidelinePath(
        $axis,
        $value,
        $depth,
        &$x,
     ...
     * @param mixed $key
     */
    protected function updateAndUnset(&$var, &$array, $key): void
    {
        if (isset($array[$key])) {
            $var = $array[$key];
            unset($array[$key]);
