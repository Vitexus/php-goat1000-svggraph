...
abstract class GridGraph extends Graph
{
    ...
    public function getAxis($axis, $num)
    {
        ...
        if (isset($this->{$axis_var}, $this->{$axis_var}[$num])) {
     ...
     *
     * @param mixed $x_overlap
     * @param mixed $y_overlap
     */
     ...
     */
    protected function labelAdjustment(): void
    {
        $grid_l = $grid_t = $grid_r = $grid_b = null;

        $grid_set = $this->getOption(
            'grid_left',
            'grid_right',
            'grid_top',
            'grid_bottom',
        ...
        for ($i = 0; $i < 10; ++$i) {
            ...
            [$extra_r, $extra_t] = $this->adjustAxes($x_len, $y_len);
            [$x_axes, $y_axes] = $this->getAxes($ends, $x_len, $y_len);
     ...
     *
     * @param mixed $length_x
     * @param mixed $length_y
     * @param mixed $x_axes
     * @param mixed $y_axes
     */
     ...
     */
    protected function setGridDimensions(): void
    {
    ...
    protected function getAxisEnds()
    {
        ...
        if ($this->guidelines !== null) {
            [$g_min_x, $g_min_y, $g_max_x, $g_max_y] = $this->guidelines->getMinMax();
        }
        ...
        for ($i = 0; $i < $y_axis_count; ++$i) {
            [$fixed_min, $fixed_max] = $this->getFixedAxisOptions('y', $i);
        ...
        for ($i = 0; $i < $x_axis_count; ++$i) {
            [$fixed_min, $fixed_max] = $this->getFixedAxisOptions('x', $i);
    ...
    protected function createXAxis($factory, $length, $ends, $i, $min_space, $grid_division)
    {
        ...
        $decimal_digits = $this->getOption(
            ['decimal_digits_x', $i],
            'decimal_digits',
        );
        $text_callback = $this->getOption(
            ['axis_text_callback_x', $i],
            'axis_text_callback',
        );
        $values = $this->multi_graph ?: $this->values;
    ...
    protected function getAxes($ends, &$x_len, &$y_len)
    {
        ...
        for ($i = 0; $i < $x_axis_count; ++$i) {
            $min_space = $this->getOption(
                ['minimum_grid_spacing_h', $i],
     ...
     */
    protected function validateAxisOptions(): void
    {
     ...
     */
    protected function calcAxes(): void
    {
     ...
     */
    protected function calcGrid(): void
    {
    ...
    protected function axes()
    {
        ...
        foreach ($this->x_axes as $axis_no => $axis) {
            if ($axis !== null) {
                ...
                [$x, $y] = $this->getAxisLocation('h', $axis_no);
                $axes .= $display_axis->draw(
                    $x,
                    $y,
                    $this->pad_left,
                    $this->pad_top,
        ...
        foreach ($this->y_axes as $axis_no => $axis) {
            if ($axis !== null) {
                $display_axis = $this->getDisplayAxis($axis, $axis_no, 'v', 'y');
                [$x, $y] = $this->getAxisLocation('v', $axis_no);
                $axes .= $display_axis->draw(
                    $x,
                    $y,
                    $this->pad_left,
     ...
     * @param mixed $attr
     */
    protected function clipGrid(&$attr): void
    {
     ...
     * @param mixed $cls
     */
    protected function calcAverages($cls = 'Goat1000\SVGGraph\Average'): void
    {
     ...
     */
    protected function calcGuidelines(): void
    {
        $this->calcAverages();
