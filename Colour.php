...
class Colour implements \Stringable
{
    ...
    public function __construct(
        ...
        if (\is_object($colour) && $colour::class === 'Goat1000\\SVGGraph\\Colour') {
    ...
    public function __toString()
    {
        ...
        if ($this->gradient) {
            $gradient_id = $this->graph->defs->addGradient(
                $this->colour,
                $this->key,
                $this->radial,
            );
     ...
     * @param mixed $key
     */
    public function setGradientKey($key): void
    {
        $this->key = $key;
    }
    ...
    public function solid()
    {
        ...
        [$solid] = $this->extractOpacity($this->colour[0]);
     ...
     * @param mixed $colour
     */
    private function extract($colour): void
    {
        $filters = '';
        $fpos = strpos($colour, '/');
        ...
        [$colour, $opacity] = $this->extractOpacity($colour);
