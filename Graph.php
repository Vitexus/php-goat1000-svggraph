...
abstract class Graph
{
    ...
    public function values($values): void
    {
        ...
        if ($datetime_keys && $datetime_key_format) {
            self::$key_format = $datetime_key_format;
        }

        if ($structured_data || \is_array($structure)) {
            $this->setOption('structured_data', true);
            $this->values = new StructuredData(
                $new_values,
                $force_assoc,
                $datetime_keys,
                $structure,
                $this->getOption('repeated_keys'),
                $this->getOption('sort_keys'),
     ...
     *
     * @param mixed $subgraphs
     */
    public function subgraphs($subgraphs): void
    {
     ...
     * Set up the colours.
     */
    public function colours(Colours $colours): void
    {
    ...
    public function getJavascript()
    {
        if (!isset(self::$javascript)) {
            self::$javascript = new Javascript($this);
        }
    ...
    public function parsePosition($pos, $w = 0, $h = 0, $pad = 0)
    {
        ...
        if (\count($parts)) {
            ...
            $inner_at = array_search('inner', $parts, true);
            $outer_at = array_search('outer', $parts, true);
        ...
        while (\count($parts)) {
            ...
            switch ($part) {
                ...
                case 'right' : $hpos = $inside ? 'r' : 'or';
                    ...
                    break;
                case 'above' : $inside = false;
                    $vpos = 'ot';

                    break;
                case 'below' : $inside = false;
                    $vpos = 'ob';
    ...
    public static function relativePosition(
        ...
        [$hpos, $vpos, $offset_x, $offset_y] = self::translatePosition($pos);
        ...
        switch ($vpos) {
            case 'ot': $y = $top - $height - $pad;

                break;
            case 't': $y = $top + $pad;

                break;
            case 'b': $y = $bottom - $height - $pad;

                break;
            case 'ob': $y = $bottom + $pad;
        ...
        switch ($hpos) {
            case 'ol': $x = $left - $width - $pad;

                break;
            case 'l': $x = $left + $pad;

                break;
            case 'r': $x = $right - $width - $pad;
    ...
    public function element(
        ...
        if ($this->namespace && !str_contains($name, ':')) {
            $name = 'svg:'.$name;
        }
    ...
    public function getLinkURL($item, $key, $row = 0)
    {
        ...
        if ($link !== null && !str_contains($link, '//')) {
     ...
     * @param mixed $key
     * @param mixed $dataset
     * @param mixed $allow_gradient
     * @param mixed $allow_pattern
     */
     ...
     */
    public function setOption($name, $value, $index = null): void
    {
     ...
     */
    public function addBackMatter($fragment): void
    {
    ...
    {
        [$hpos, $vpos] = self::translatePosition($pos);

        return !str_contains($hpos.$vpos, 'o');
    }
    ...
    public function dataLabelStyle($dataset, $index, &$item)
    {
        ...
        $style['pad_x'] = $this->getOption(
            ['data_label_padding_x', $dataset],
            ['data_label_padding', $dataset],
        );
        $style['pad_y'] = $this->getOption(
            ['data_label_padding_y', $dataset],
            ['data_label_padding', $dataset],
        );
    ...
    public function fetchJavascript($cdata = true, $no_namespace = true, $nonce = null)
    {
        ...
        $script = self::$javascript->getCode($cdata, $this->getOption('minify_js'));
    ...
    public static function dateConvert($k)
    {
        ...
        if (self::$key_format !== null) {
            $dt = date_create_from_format(self::$key_format, $k);
        }
    ...
    {
        $ini_file = __DIR__.\DIRECTORY_SEPARATOR.'svggraph.ini';
        $ini_settings = false;
     ...
     * @param mixed $index
     * @param mixed $item
     * @param mixed $style_info
     */
    protected function setLegendEntry($dataset, $index, $item, $style_info): void
    {
        if ($this->legend === null) {
            return;
        }
    ...
    protected function getTitle()
    {
        ...
        if ($svg_subtext->strlen($info['stitle']) > 0) {
            $info['sfont_size'] = Number::units($this->getOption(
                'graph_subtitle_font_size',
                'graph_title_font_size',
            ));
            $info['sweight'] = $this->getOption(
                'graph_subtitle_font_weight',
                'graph_title_font_weight',
            );
            $info['scolour'] = $this->getOption(
                'graph_subtitle_colour',
                'graph_title_colour',
            );
            $info['sspace'] = $this->getOption(
                'graph_subtitle_space',
                'graph_title_space',
            );
            ...
            [$swidth, $sheight] = $svg_subtext->measure(
                $info['stitle'],
                $info['sfont_size'],
                0,
                $info['sline_spacing'],
    ...
    protected function backgroundImage($shadow, $clip_path)
    {
        ...
        $image = [
            ...
            'preserveAspectRatio' => ($mode === 'stretch' ? 'none' : 'xMinYMin'),
        ];
    ...
    protected function contrastText(
        ...
        $bg = $this->element(
            'text',
            ['stroke-width' => '2px', 'stroke' => $bcolour],
            null,
            $text,
        );
     ...
     * @param mixed      $reverse
     */
    protected function colourSetup($count, $datasets = null, $reverse = false): void
    {
     ...
     * @param mixed      $key
     * @param mixed      $dataset
     * @param null|mixed $line_join
     */
    protected function setStroke(&$attr, &$item, $key, $dataset, $line_join = null): void
    {
    ...
    protected function setTooltip(
        ...
        if (\is_callable($callback)) {
            ...
            $text = $callback(...[$dataset, $key, $value]);
    ...
    {
        $this->getDataLabels()->addContentLabel(
            $dataset,
            $index,
            $x,
            $y,
