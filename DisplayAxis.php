...
class DisplayAxis
{
    ...
    public function __construct(
        ...
        $get_axis_option = static function ($option) use ($graph, $o, $axis_no) {
        ...
        if ($this->show_label) {
            $styles['l_font'] = $graph->getOption(
                ['label_font_'.$o, $axis_no],
                'label_font',
                ['axis_font_'.$o, $axis_no],
                'axis_font',
            );
            $styles['l_font_size'] = Number::units($graph->getOption(
                ['label_font_size_'.$o, $axis_no],
                'label_font_size',
                ['axis_font_size_'.$o, $axis_no],
                'axis_font_size',
            ));
            $styles['l_font_weight'] = $graph->getOption(
                ['label_font_weight_'.$o, $axis_no],
                'label_font_weight',
            );
            $styles['l_colour'] = new Colour($graph, $graph->getOption(
                ['label_colour_'.$o, $axis_no],
                'label_colour',
                ['axis_text_colour_'.$o, $axis_no],
                'axis_text_colour',
                ['@', $styles['colour']],
    ...
    public function getTextOverlap()
    {
        ...
        [$x_off, $y_off, $opp] = $this->getTextOffset(0, 0, 0, 0, 0, 0, $level);
    ...
    public function drawSubDivisions($x, $y, $g_width, $g_height)
    {
        ...
        $points = $this->axis->getGridSubdivisions(
            $this->minimum_subdivision,
            $this->minimum_units,
            0,
            $this->subdivisions_fixed,
        );
        $d = $this->getDivisionPath($x, $y, $points, $path_info, 0);
        $attr = [
            'd' => $d,
    ...
    public function drawText($x, $y, $gx, $gy, $g_width, $g_height)
    {
        ...
        if ($this->show_text) {
            [$x_offset, $y_offset, $opposite] = $this->getTextOffset(
                $x,
                $y,
                $gx,
                $gy,
     ...
     * @param mixed $level
     */
    protected function getTextOffset(
        $ax,
        $ay,
        $gx,
        $gy,
