...
class Axis
{
     ...
     * @param mixed $allow_zero
     * @param mixed $prefer_zero
     */
    public static function calcMinimum(
        $min_value,
        $max_value,
        $allow_zero,
        $prefer_zero,
     ...
     * @param mixed $prefer_zero
     */
    public static function calcMaximum(
        $min_value,
        $max_value,
        $allow_zero,
        $prefer_zero,
     ...
     * @param mixed $t
     */
    public function setTightness($t): void
    {
        $this->tightness = $t;
    }
    ...
    protected function grid()
    {
        ...
        $magnitude = max(10 ** floor(log10($scale)), $this->min_unit);
    ...
    protected function getGridPoint($position, $value)
    {
        ...
        if (\is_callable($this->label_callback)) {
            ...
            $text = ($this->label_callback)($value, $key);
     ...
     * @param mixed $min
     * @param mixed $count
     * @param mixed $neg_count
     * @param mixed $magnitude
     */
    private function findDivision($length, $min, &$count, &$neg_count, &$magnitude): void
    {
    ...
    private function findSubdiv($grid_div, $min, $min_unit, $fixed)
    {
        ...
        $D1 = (int) round(100 * (10 ** (-floor(log10($D)))) * $D);
