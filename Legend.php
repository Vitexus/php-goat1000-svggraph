...
class Legend
{
    ...
    public function __construct(&$graph)
    {
        ...
        $this->title_colour = $graph->getOption(
            'legend_title_colour',
            'legend_colour',
        );
        $this->title_font = $graph->getOption('legend_title_font', 'legend_font');
        $this->title_font_adjust = $graph->getOption(
            'legend_title_font_adjust',
            'legend_font_adjust',
        );
        $this->title_font_size = Number::units($graph->getOption(
     ...
     */
    public function setEntry($dataset, $index, $item, $style_info): void
    {
        ...
        if ($text !== '') {
            $this->entry_details[$entry] = new LegendEntry(
                $item,
                $text,
                $link,
                $style_info,
    ...
    public function draw()
    {
        ...
        foreach ($this->entry_details as $entry) {
            [$w, $h] = $svg_text->measure($entry->text, $font_size, 0, $this->line_spacing);
        ...
        if ($this->title !== '') {
            ...
            $svg_text_title = new Text(
                $this->graph,
                $this->title_font,
                $this->title_font_adjust,
            );
            [$tw, $th] = $svg_text_title->measure(
                $this->title,
                $title_font_size,
                0,
                $this->title_line_spacing,
            );
            $title_width = $tw + $padding_x * 2;
            $start_y += $th + $this->graph->getOption(
                'legend_title_spacing',
                'legend_padding',
                1,
            );
        }
        ...
        $col_space = $this->graph->getOption(
        ...
        foreach ($entries as $entry) {
            ...
            $element = $this->graph->drawLegendEntry(
                $x,
                $e_y,
                $w,
                $this->entry_height,
        ...
        if (\is_array($this->position)) {
            ...
            [$tx, $ty] = $c->transformCoords($px, $py);

            // handle relative positions
            switch ($vx['value']) {
                case 'c': $left = $tx - ($width / 2);

                    break;
                case 'r': $left = $tx - $width;
            ...
            switch ($vy['value']) {
                case 'c' : $top = $ty - ($height / 2);

                    break;
                case 'b' : $top = $ty - $height;
            ...
            [$left, $top] = $this->graph->parsePosition(
                $this->position,
                $width,
                $height,
            );
    ...
    protected function filterEntries($entries)
    {
        ...
        if (\is_array($filtered)) {
            ...
            foreach ($filtered as $e) {
                if (!\is_object($e) || $e::class !== 'Goat1000\\SVGGraph\\LegendEntry') {
                    $valid = false;
                }
            }
        }
    ...
    protected function getEntries()
    {
        ...
        if (!empty($entry_order)) {
            if (str_contains($entry_order, 'sort')) {
            ...
            if (str_contains($entry_order, 'reverse')) {
