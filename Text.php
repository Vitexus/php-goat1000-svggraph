...
class Text
{
    ...
    private static $use_iconv;
    private static $use_mbstring;

    public function __construct(&$graph, $font = null, $adjust = 0)
    {
        // iconv can be troublesome
        if (self::$use_iconv === null) {
            self::$use_iconv = false;
        ...
        if (self::$use_mbstring === null) {
            self::$use_mbstring = $graph->getOption('use_mbstring', true)
    ...
    public function measure($text, $font_size, $angle = 0, $line_spacing = 0)
    {
        ...
        foreach ($lines as $l) {
            [$lw, $lh] = $this->measureLine($l, $font_size);
     ...
     * @param mixed      $anchor
     * @param mixed      $angle
     * @param null|mixed $rcx
     * @param null|mixed $rcy
     */
    ...
    public function baseline($font_size)
    {
        ...
        if ($metrics) {
            $metrics = &self::$metrics[$this->metrics_file];
    ...
    public function text($text, $line_spacing, $attribs, $styles = null)
    {
        ...
        $text = preg_replace(
            '/&amp;(amp|apos|gt|lt|quot|#x[a-fA-F0-9]+|#\d+);/u',
            '&$1;',
            $text,
        );
    ...
    {
        if (self::isCombiner($char)) {
            return 0;
        }

        if (self::isDoubleWidth($char)) {
            return 2;
        }
    ...
    public function replaceEntities($text)
    {
        ...
        return preg_replace_callback('/&#(x)?([a-fA-F0-9]+);/ui', static function ($ent) {
    ...
    private function measureCached($text, $font_size, $angle, $line_spacing)
    {
        ...
        return self::$measure_cache[$key] ?? null;
    }
     ...
     * @param mixed $height
     */
    private function cacheMeasurement(
        $text,
        $font_size,
        $angle,
        $line_spacing,
        ...
        $key = $this->getCacheKey($text, $font_size, $angle, $line_spacing);
        self::$measure_cache[$key] = [$width, $height];
    }
    ...
    {
        return $text.'-'.$this->font.'-'.$this->adjust.'-'.$font_size.
          '-'.$angle.'-'.$line_spacing;
    }
     ...
     * @param mixed $text
     */
    private static function splitChars($text)
    {
    ...
    private function convert($text)
    {
        if ($this->encoding !== 'UTF-8') {
            ...
            if (!$converted && self::$use_mbstring) {
    ...
    private static function loadMetrics($font)
    {
        ...
        $filename = self::metricsFilename($font);

        if (isset(self::$metrics[$filename])) {
            return self::$metrics[$filename] !== false;
        }
        ...
        if (file_exists($metrics_path)) {
            ...
            if (\is_array($metrics)) {
                if (isset($metrics['map'])) {
                    ...
                    if (isset(self::$metrics[$map_to])) {
                        $metrics = self::$metrics[$map_to];
                    } else {
