...
class RGBColour
{
    ...
    public function __construct($colour)
    {
        ...
        if (isset(self::KEYWORDS[$colour])) {
            $colour = self::KEYWORDS[$colour];
        }
        ...
        if (!str_starts_with($colour, '#')) {
            $c = self::fromRGB($colour);

            if ($c === null) {
                $c = self::fromRGBA($colour);
            }

            if ($c === null) {
                $c = self::fromHSL($colour);
            }

            if ($c === null) {
                $c = self::fromHSLA($colour);
            }
        }
    ...
    {
        if (\strlen($colour) !== 4 || !str_starts_with($colour, '#')) {
            return null;
        }
     ...
     * @param null|mixed $a
     */
    public function setRGB($r, $g, $b, $a = null): void
    {
        $this->r = $r;
        $this->g = $g;
        $this->b = $b;
     ...
     * @param null|mixed $a
     */
    public function setHSL($h, $s, $l, $a = null): void
    {
        [$r, $g, $b] = self::hslToRgb($h, $s, $l);
        $this->setRGB($r, $g, $b, $a);
    }
