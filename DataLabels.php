...
class DataLabels
{
     ...
     * @param null|mixed $fade_in
     * @param null|mixed $click
     */
     ...
     * @param mixed $content
     */
    public function addContentLabel($dataset, $index, $x, $y, $w, $h, $content): void
    {
        if (!isset($this->labels[$dataset])) {
            $this->labels[$dataset] = [];
        }
     ...
     *
     * @param mixed $label_array
     */
    public function addUserLabel($label_array): void
    {
            ...
            foreach ($label_set as $i => $label) {
                if ($this->filter($set_filter, $dataset, $label, $i)) {
                    ...
                    if ($content !== null && $content !== '') {
                        [$w, $h] = $this->measureLabel($content, $dataset, $i, $label);
     ...
     * @param mixed $value
     */
    protected function setupFilters($dataset, $index, $value): void
    {
     ...
     * @param mixed $labels
     */
    protected function setLabelSizes(&$labels): void
    {
        if (!$this->same_size) {
            return;
        }
    ...
    protected function measureLabel($content, $dataset, $index, &$gobject)
    {
        ...
        // get size of text
        [$font_size, $line_spacing] = $this->getFontSize($style);
    ...
    protected function drawLabel(
        ...
        if ($dataset === '_user') {
            ...
            if ($pos === null) {
                ...
                [$pos, $target] = $this->graph->dataLabelPosition(
                    $dataset,
                    $index,
                    $gobject['item'],
                    $gobject['x'],
        ...
        [$x, $y, $anchor, $hpos, $vpos] = Graph::relativePosition(
            $pos,
            $gobject['y'],
            $gobject['x'],
        ...
        [$colour, $back_colour] = $this->getColours($hpos, $vpos, $style);
        [$font_size, $line_spacing] = $this->getFontSize($style);
        $text = [
        ...
        $svg_text = new Text($this->graph, $style['font'], $style['font_adjust']);
        [$tbw, $tbh] = $svg_text->measure($content, $font_size, 0, $line_spacing);
     ...
     * @param mixed $dataset
     */
    protected function itemStyles(&$style, &$item, $index, $dataset): void
    {
     ...
     * @param mixed $label_array
     */
    protected function userStyles(&$style, &$label_array): void
    {
    ...
    protected function filter($filter, $dataset, &$label, $index)
    {
        ...
        $tests = [
            ...
            'nonzero' => static function () use (&$item) {
                return $item->value !== 0;
            },
            'none' => static function () use (&$item) {
    ...
    protected function lineLabel($x, $y, $w, $h, &$style, &$surround)
    {
        ...
        if ($style['tail_length'] === 'auto') {
            ...
            [$font_size] = $this->getFontSize($style);
    ...
    {
        if ($style['round']) {
            [$cx, $cy, $bbradius] = $this->calcRoundLabel($x, $y, $w, $h);
            [$target, $angle, $len] = $this->getRoundTailTarget(
                $style,
                $cx,
                $cy,
                $bbradius,
            ...
            [$target, $angle, $len] = $this->getBoxTailTarget(
                $style,
                $x,
                $y,
                $x + $w,
    ...
    protected function boxLineLabel($x, $y, $w, $h, &$style, &$surround)
    {
            ...
            if ($angle > \M_PI) {
                ...
                if ($round) {
                    if (!$c_tl->isEmpty() && $p1->x < $x1c) {
                        $cross = self::lineCrossArc($p1->x, $p1->y, $angle, $x1c, $y1c, $round, 'tl');
                        ...
                        if ($p2->x < $x1c) {
                            $cross = self::lineCrossArc($p2->x, $p2->y, $angle, $x1c, $y1c, $round, 'tl');
                    ...
                    if (!$c_tr->isEmpty() && $x2c < $p2->x) {
                        $cross = self::lineCrossArc($p2->x, $p2->y, $angle, $x2c, $y1c, $round, 'tr');
                        $end = new PathData($arc);
                        $end->add($cross);
                        $c_tr = new PathData('L', $x2, $y1c);

                        if ($x2c < $p1->x) {
                            $cross = self::lineCrossArc($p1->x, $p2->y, $angle, $x2c, $y1c, $round, 'tr');
                            $start = new PathData('M');
                            $start->add($cross);
                            $start->add($arc);
                            $start->add($x2c, $y1);
                ...
                if ($round) {
                    if (!$c_bl->isEmpty() && $p2->x < $x1c) {
                        $cross = self::lineCrossArc($p2->x, $p2->y, $angle, $x1c, $y2c, $round, 'bl');
                        $end = new PathData($arc);
                        $end->add($cross);
                        $c_bl = new PathData('L', $x1, $y2c);

                        if ($p1->x < $x1c) {
                            $cross = self::lineCrossArc($p1->x, $p1->y, $angle, $x1c, $y2c, $round, 'bl');
                            $start = new PathData('M');
                            $start->add($cross);
                            $start->add($arc);
                            $start->add($x1c, $y2);
                    ...
                    if (!$c_br->isEmpty() && $x2c < $p1->x) {
                        $cross = self::lineCrossArc($p1->x, $p1->y, $angle, $x2c, $y2c, $round, 'br');
                        $start = new PathData('M');
                        $start->add($cross);
                        ...
                        if ($x2c < $p2->x) {
                            $cross = self::lineCrossArc($p2->x, $p2->y, $angle, $x2c, $y2c, $round, 'br');
            ...
            if ($angle < \M_PI * 0.5 || $angle > \M_PI * 1.5) {
                ...
                if ($round) {
                    ...
                    if (!$c_tr->isEmpty() && $p1->y < $y1c) {
                        $cross = self::lineCrossArc($p1->x, $p1->y, $angle, $x2c, $y1c, $round, 'tr');
                        $start = new PathData('M');
                        $start->add($cross);
        ...
        $box_path->add($this->getTailEnding(
            $target[0],
            $target[1],
            $angle,
            $t_width,
     ...
     * @param mixed $corner
     */
    private static function lineCrossArc($x, $y, $angle, $cx, $cy, $radius, $corner)
    {
        $h = $cx;
        $k = $cy;
        $r = $radius;
     ...
     * @param mixed $h
     */
    private static function makeSquare(&$x, &$y, &$w, &$h): void
    {
        if ($w === $h) {
            return;
        }
