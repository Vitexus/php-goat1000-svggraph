...
class GanttChart extends HorizontalBarGraph
{
    ...
    public function values($values)
    {
        ...
        $update_times = static function ($item) use (&$start_date, &$end_date, $ts) {
            if (!isset($item->value)) {
                return;
            }
        ...
        $fix_groups = static function ($item) use ($groups, $item_groups) {
            if (!isset($groups[$item->key])) {
    ...
    protected function drawBar(
        ...
        if ($item->milestone) {
            ...
            $complete = new Number($item->complete ?: 0);
            $label = "[{$complete}%]";
            $label_shown = $this->addDataLabel(
                $dataset,
                $index,
    ...
    {
        [$colour_incomplete, $colour_complete] = $this->getBarColours($item, $index, $dataset);
    ...
    protected function drawDependencies(&$item, $index, $dataset, $bar)
    {
        ...
        $head_size = $this->getItemOption(
            'gantt_depends_head_size',
            $dataset,
            $item,
            'depends_head_size',
        );
        $stroke_width = min(10, max(
            0.1,
        ...
        foreach ($depends as $k => $d) {
            ...
            $arrow = new GanttArrow(
                new Point($dbar['x'], $dbar['y']),
    ...
    protected function formatTooltip(&$item, $dataset, $key, $value)
    {
        ...
        if ($item->milestone) {
            $ttext = $item->axis_text ?: $key;
            $ttext .= "\n".$text_start;

            return $ttext;
        }

        $pluralize = static function ($n, $units) {
     ...
     * @param mixed $items
     */
    private function autoHeight($items): void
    {
        $axis = null;
     ...
     * @param mixed $ends
     */
    private function autoFormatAxis($ends): void
    {
