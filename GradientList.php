...
class GradientList
{
    ...
    public function getColour($key, $position)
    {
        ...
        if (\in_array($colours[\count($colours) - 1], ['h', 'v', 'r'], true)) {
        ...
        $make_colour = static function ($r, $g, $b, $o) {
            $str = "rgb({$r},{$g},{$b})";
        ...
        foreach ($segments as $seg) {
            if (abs($seg[0] - $position) < 0.1) {
                ...
                return new Colour($this->graph, $c_string);
            }
        ...
        $c0 = $seg0[1]->rgb();
        $c1 = $seg1[1]->rgb();
        $vr = self::calcComponent($c0[0], $c1[0], $seg0[0], $seg1[0], $position);
        $vg = self::calcComponent($c0[1], $c1[1], $seg0[0], $seg1[0], $position);
        $vb = self::calcComponent($c0[2], $c1[2], $seg0[0], $seg1[0], $position);

        $o0 = $seg0[2] === null ? 1 : $seg0[2];
        $o1 = $seg1[2] === null ? 1 : $seg1[2];
        $vo = self::calcComponent($o0, $o1, $seg0[0], $seg1[0], $position);
     ...
     * @param mixed $defs
     */
    public function makeGradients(&$defs): void
    {
     ...
     * @param mixed $d
     */
    private static function calcComponent($c0, $c1, $o0, $o1, $d)
    {
