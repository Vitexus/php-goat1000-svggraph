 ...
 */
class Matrix implements \Stringable
{
    public const MAX_SIZE = 100000;
    private $rows = 0;
    private $cols = 0;
    private $data = [];
     ...
     * @param mixed $bcscale
     */
    public function add(self $m, $bcscale = 50)
    {
     ...
     * @param mixed $bcscale
     */
    public function subtract(self $m, $bcscale = 50)
    {
        if ($m->rows !== $this->rows || $m->cols !== $this->cols) {
     ...
     * @param mixed $bcscale
     */
    public function multiply(self $m1, $bcscale = 50)
    {
     ...
     * @param mixed $bcscale
     */
    public function gaussian($bcscale = 50): void
    {
        $old_scale = bcscale($bcscale);

        $argmax = static function ($a, $b, $m, $col) {
            $max = 0;
            $max_i = 0;
     ...
     * @param mixed $bcscale
     */
    public function gaussian_solve(self $rhs, $bcscale = 50)
    {
        $a = $this->augment($rhs);
        $a->gaussian($bcscale);
     ...
     */
    public function augment(self $rhs)
    {
        $m = $this->rows;
     ...
     * @param mixed $bcscale
     */
    public function solve(self $a, $bcscale)
    {
        $result = new self(1, $a->rows);
        $old_scale = bcscale($bcscale);
     ...
     * @param mixed $r2
     */
    public function rowSwap($r1, $r2): void
    {
