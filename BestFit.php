...
class BestFit
{
     ...
     * @param mixed $points
     */
    public function add($dataset, $points): void
    {
        ...
        [$start, $end] = $this->getRange($dataset);
    ...
    protected function filterPoints($points, $start, $end)
    {
        ...
        if ($start === null) {
            $callback = static function ($p) use ($end) {
                return $p->x <= $end;
            };
        } elseif ($end === null) {
            $callback = static function ($p) use ($start) {
                return $p->x >= $start;
            };
        } else {
            $callback = static function ($p) use ($start, $end) {
                return $p->x <= $end && $p->x >= $start;
            };
        }
     ...
     *
     * @param mixed $dataset
     * @param mixed $line_path
     * @param mixed $proj_path
     */
